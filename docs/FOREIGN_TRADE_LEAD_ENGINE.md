...
在 Supabase SQL Editor 按既有迁移顺序执行 `backend/supabase/`。已使用 V1.13～V1.26 的环境请继续执行：

```sql
-- backend/supabase/v1_27_foreign_trade_lead_engine.sql
-- backend/supabase/v1_28_pure_crawler_discovery_strategy.sql
```

V1.27 新增 `product_keywords`、`crawl_sources` 和 `domain_blocklist`，并扩展既有 `lead_search_tasks`、`lead_discovery_runs`、`customer_leads`。V1.28 为任务增加 `discovery_strategy`，默认是 `public_seed_crawl`（纯爬虫）。所有新表启用 RLS，`owner_user_id = auth.uid()`；API 始终携带用户 JWT，因此普通用户不能读取别人的规则、来源或黑名单。`customer_leads` 继续沿用原有 `user_id` RLS。
...
3. 通过 `POST /api/sources` 保存展会、协会、行业目录或用户导入来源；也可在任务中填公开目录 URL。三款重点产品已内置一批经人工复核的制造商官网和协会入口。
4. 新建任务并选择产品、目标国家及候选数量。默认“纯爬虫”只从公开目录、协会会员页与官网种子开始，不依赖 GPT、Brave 或任何搜索 Key。
...
- `BRAVE_SEARCH_API_KEY`：纯爬虫模式不使用，可留空；系统不会抓取 Google/Bing 等搜索结果页。
...
- **任务没有发现候选**：补充目标国家的公开协会、展会或行业目录 URL，或导入自有官网种子 CSV；纯爬虫模式不会抓取 Google/Bing 页面。
