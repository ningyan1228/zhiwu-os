...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {view === 'dashboard' && <Dashboard customers={customers} today={today} onOpenCRM={() => setView('crm')} openCustomer={setSelected} />}
...
function Dashboard({ customers, today, onOpenCRM, openCustomer }: { customers: Customer[]; today: string; onOpenCRM: () => void; openCustomer: (customer: Customer) => void }) {
  const reminders = [
    ['Agrileaf', 'Waiting payment', '3 days', 'warning'], ['Flexo', 'Waiting customer sample', '5 days', 'attention'],
    ['Inkofix', 'Quotation follow-up', '7 days', 'attention'], ['ATSajan', 'Maintain relationship', '', 'success'],
  ] as const
  const tasks = ['向 Uflex 发送 25kg 样品 PI', '确认 Agrileaf 的 USD 150 样品付款', '跟进 Flexo 客户样品的寄送状态']
  const pipeline = [['New Inquiry', 6], ['Technical Discussion', 4], ['Quotation', 2], ['Sample Trial', 3], ['Negotiation', 1], ['Won', 0]]
  const findCustomer = (name: string) => customers.find(customer => customer.company_name === name)
  return <section className="page dashboard"><div className="hero"><div><p className="eyebrow"><span className="pulse"/> {today}</p><h1>早上好，Zhiwu <span>✦</span></h1><p className="subhead">一眼看清客户状态、项目阶段与下一步商业机会。</p></div><div className="goal"><div className="goal-top"><span>2026 年收入目标</span><b>¥1,000,000</b></div><div className="progress"><i style={{width:'23%'}}/></div><div className="goal-foot"><span>已完成 ¥230,000</span><strong>23%</strong></div></div></div>
    <p className="section-kicker">TODAY'S SALES STATUS</p><div className="metrics metrics-six"><Metric label="客户总数" value={String(customers.length)} delta="6 个真实业务机会" icon={<Users/>}/><Metric label="进行中项目" value="6" delta="聚焦本周推进" icon={<Layers3/>}/><Metric label="报价中" value="2" delta="1 个待价格沟通" icon={<ClipboardList/>}/><Metric label="样品阶段" value="3" delta="2 笔付款待确认" icon={<Package/>}/><Metric label="待跟进客户" value="5" delta="最早 3 天后到期" icon={<Bell/>}/><Metric label="产品数量" value="5" delta="已关联客户项目" icon={<Sparkles/>}/></div>
    ...
    <section className="panel reminders"><PanelTitle title="Follow-up Reminder" action="进入 CRM" onAction={onOpenCRM}/>{reminders.map(([name, note, days, tone]) => { const customer = findCustomer(name); return <button className="reminder-row" key={name} onClick={() => customer ? openCustomer(customer) : onOpenCRM()}><span className={`reminder-dot ${tone}`}>{tone === 'success' ? '●' : '⚠'}</span><div><b>{name}</b><span>{note}</span></div>{days && <time>{days}</time>}<ChevronRight size={16}/></button> })}</section></div>
    <section className="panel activity"><PanelTitle title="最近动态" action="查看全部"/><div className="timeline"><Activity icon={<Users/>} title="Uflex 已确认性能指标" text="25kg 样品需求，等待 PI 与付款确认" time="今天"/><Activity icon={<Package/>} title="Flexo 等待客户原料样品" text="E4050 替代项目进入测试准备阶段" time="昨天"/><Activity icon={<Check/>} title="Inkofix 已收到 CIF Mundra 报价" text="下一步确认热封工艺与强度要求" time="8月15日"/></div></section>
  </section>
}
...
function CRM({customers,query,setQuery,open,create}:{customers:Customer[];query:string;setQuery:(v:string)=>void;open:(c:Customer)=>void;create:()=>void}) { return <section className="page crm"><div className="page-heading"><div><p className="eyebrow">EXTERNAL TRADE CONTROL CENTER</p><h1>外贸 CRM</h1><p>以客户状态、项目阶段和下一步行动推进每一笔真实业务机会。</p></div><button className="primary" onClick={create}><Plus size={17}/> 新建客户</button></div><div className="crm-kpis"><span><b>{customers.length}</b> 客户总数</span><span><b>6</b> 进行中项目</span><span><b>2</b> 报价中</span><span><b>3</b> 样品阶段</span><span><b>5</b> 待跟进</span></div><div className="crm-stage-strip"><span>New Inquiry <b>6</b></span><span>Technical Discussion <b>4</b></span><span>Quotation <b>2</b></span><span>Sample Trial <b>3</b></span><span>Negotiation <b>1</b></span></div><div className="table-panel"><div className="table-tools"><label><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜索公司、联系人或国家" /></label><button className="filter">阶段 <ChevronRight size={15}/></button><button className="filter">国家 <ChevronRight size={15}/></button></div><div className="table-wrap"><table><thead><tr><th>客户 / 项目</th><th>状态</th><th>产品与应用</th><th>优先级</th><th>下一步行动</th><th/></tr></thead><tbody>{customers.map(c=><tr key={c.id} onClick={()=>open(c)}><td><div className="customer-cell"><div className="company-avatar">{c.company_name[0]}</div><div><b>{c.company_name}</b><span>{c.contact_person} · {c.country} · {stageLabels[c.customer_stage]}</span></div></div></td><td><div className="status-stack"><Stage stage={c.customer_stage}/><span className={`status-label ${c.status_tone ?? ''}`}>{c.status_label ?? '跟进中'}</span></div></td><td><b className="product-interest">{c.product_interest}</b><span className="application-cell">{c.application ?? '待补充应用'}</span></td><td><Priority value={c.priority}/></td><td><span className="next-action-cell">{c.next_action?.[0] ?? '安排下一次客户联系'}</span><small>{c.next_followup_date}</small></td><td><ChevronRight size={17}/></td></tr>)}</tbody></table>{!customers.length && <div className="empty">还没有客户，点击“新建客户”开始添加。</div>}</div></div></section> }
...
function CustomerDrawer({customer,close}:{customer:Customer;close:()=>void}) {
  ...
  return <div className="drawer-layer" onMouseDown={close}><aside className="drawer" onMouseDown={e=>e.stopPropagation()}><button className="close" onClick={close}><X size={20}/></button>
    ...
    <DetailSection title="Project Information"><div className="project-summary"><span>当前状态</span><b className={`status-label ${customer.status_tone ?? ''}`}>{customer.status_label ?? '跟进中'}</b><span>项目阶段</span><b>{stageLabels[customer.customer_stage]}</b>{customer.monthly_consumption && <><span>月度用量</span><b>{customer.monthly_consumption}</b></>}</div>{customer.project_background && list(customer.project_background)}{customer.requirements && <div className="requirement-box"><span>关键需求</span>{list(customer.requirements)}</div>}</DetailSection>
    <DetailSection title="Product"><div className="product-detail"><span>{customer.product_interest}</span><b>{customer.application ?? '应用待确认'}</b></div></DetailSection>
    ...
    <DetailSection title="Quotation History">{customer.quotation ? <div className="quotation-box">{customer.quotation.map(item => <span key={item}>{item}</span>)}</div> : <p className="detail-empty">当前没有正式报价记录。</p>}</DetailSection>
