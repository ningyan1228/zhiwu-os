...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {view === 'dashboard' && <Dashboard customers={customers} projects={projects} followups={followupRows} emails={emails} tasks={tasks} today={today} onOpenCRM={() => setView('crm')} onOpenMail={() => setView('mail')} onOpenTasks={() => setView('tasks')} openCustomer={setSelected} />}
...
function Dashboard({ customers, projects, followups, emails, tasks, today, onOpenCRM, onOpenMail, onOpenTasks, openCustomer }: { customers: Customer[]; projects: Project[]; followups: Followup[]; emails: MailEmail[]; tasks: Task[]; today: string; onOpenCRM: () => void; onOpenMail: () => void; onOpenTasks: () => void; openCustomer: (customer: Customer) => void }) {
...
function CustomerDrawer({customer,projects,quotes,followups,products,emails,tasks,close,addFollowup,addQuote,updateStage,openEmail}:{customer:Customer;projects:Project[];quotes:Quote[];followups:Followup[];products:Product[];emails:MailEmail[];tasks:Task[];close:()=>void;addFollowup:()=>void;addQuote:()=>void;updateStage:(customer:Customer,stage:CustomerStage)=>Promise<void>;openEmail:(email:MailEmail)=>void}) {
  ...
  return <div className="drawer-layer" onMouseDown={close}><aside className="drawer" onMouseDown={e=>e.stopPropagation()}><button className="close" onClick={close}><X size={20}/></button>
    ...
    <DetailSection title="工作记录">{customerTasks.length ? <div className="communication-timeline">{customerTasks.map(task => <div className="communication-item" key={task.id}><time>{task.task_date}</time><div><b>{task.status === 'Completed' ? '✓ 已完成：' : '待完成：'}{task.title}</b><span>{task.category} · {task.start_time ? timeLabel(task.start_time) : '全天'}</span></div></div>)}</div> : <p className="detail-empty">尚未为该客户安排工作任务。</p>}</DetailSection>
