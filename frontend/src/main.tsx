...
function App() {
  ...
  const [modal, setModal] = useState<'customer' | 'product' | 'followup' | 'quote' | 'task' | 'review' | 'password' | null>(null)
  ...
  return <div className="app-shell">
    ...
    {selected && <CustomerDrawer customer={selected} projects={projects} quotes={quotes} followups={followupRows} products={products} emails={emails} tasks={tasks} close={() => setSelected(null)} addFollowup={() => setModal('followup')} addQuote={() => setModal('quote')} updateStage={updateStage} openEmail={openEmail} />}
...
function CustomerDrawer({customer,projects,quotes,followups,products,emails,tasks,close,addFollowup,addQuote,updateStage,openEmail}:{customer:Customer;projects:Project[];quotes:Quote[];followups:Followup[];products:Product[];emails:MailEmail[];tasks:Task[];close:()=>void;addFollowup:()=>void;addQuote:()=>void;updateStage:(customer:Customer,stage:CustomerStage)=>Promise<void>;openEmail:(email:MailEmail)=>void}) {
