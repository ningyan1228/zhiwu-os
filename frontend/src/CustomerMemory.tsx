...
const defaultMemoryName = (customer: Customer, emails: MailEmail[]) => {
  const senderName = emails
    .filter(mail => mail.customer_id === customer.id && !mail.is_internal_sender)
    .sort((a, b) => (b.received_at || '').localeCompare(a.received_at || ''))
    .map(senderNameFromMail)
    .find(Boolean)
  const contact = senderName || customer.contact_person?.trim() || '联系人待补充'
  return `${countryName(customer.country || '地区待确认')} · ${contact}`
}
export function MemoryCRM({ customers, projects, products, followups, emails, open, create, addFollowup }: { customers: Customer[]; projects: Project[]; products: Product[]; followups: Followup[]; emails: MailEmail[]; open: (customer: Customer) => void; create: () => void; addFollowup: (customer: Customer) => void }) {
