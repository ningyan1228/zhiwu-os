...
import { ClipboardCopy, Download, FileText, MapPinned, Play, Plus, RefreshCw, Search, Trash2, Upload, X } from 'lucide-react'
import { api } from './api'
import type { ApplicationDiscoveryTask, CustomerLead, ManualPlusPacket, TdsApplication, TdsDocument, TdsPreset } from './types'
import { CustomerLeadMap } from './CustomerLeadMap'

type Props = {
  onChanged: () => Promise<void>
  onFocusLeadTask: (taskId: string) => void
}
...
export function TdsApplicationDiscovery({ onChanged, onFocusLeadTask }: Props) {
  ...
  const [plusTask, setPlusTask] = useState<ApplicationDiscoveryTask | null>(null)
  const [plusPacket, setPlusPacket] = useState<ManualPlusPacket | null>(null)
  const [plusOutput, setPlusOutput] = useState('')
  const [plusNotice, setPlusNotice] = useState('')
  ...
  const load = async (preferred?: string) => {
    ...
    const latestLinkedTask = existingTasks.find(item => item.tds_document_id === id && item.legacy_lead_task_id)
    if (latestLinkedTask?.legacy_lead_task_id) onFocusLeadTask(latestLinkedTask.legacy_lead_task_id)
  ...
  const focusTaskLeads = (task: ApplicationDiscoveryTask) => {
    if (!task.legacy_lead_task_id) return
    onFocusLeadTask(task.legacy_lead_task_id)
  }
  const runTask = (task: ApplicationDiscoveryTask) => void work(async () => {
    // If this task has run before, switch the lead library immediately. For a
    // first run the API creates the linked lead task synchronously, so select
    // it from the refreshed application task below.
    focusTaskLeads(task)
    const result = await api.runApplicationDiscoveryTask(task.id)
    const updated = await load(documentId)
    const refreshedTask = updated.find(item => item.id === task.id)
    if (refreshedTask) focusTaskLeads(refreshedTask)
    return `${result.message} 下方“分层线索库”已自动切换到本产品的需求客户，不需要再选择任务。`
  })
  const deleteTask = (task: ApplicationDiscoveryTask) => {
    const confirmed = window.confirm(`删除任务“${task.task_name}”吗？\n\n任务配置和未完成运行会删除；已经发现的客户线索与 CRM 数据会保留。`)
    if (!confirmed) return
    void work(async () => {
      const result = await api.deleteApplicationDiscoveryTask(task.id)
  ...
  const openManualPlus = async (task: ApplicationDiscoveryTask) => {
    setBusy(true); setNotice(''); setPlusNotice(''); setPlusOutput(''); setPlusTask(task); setPlusPacket(null)
    try { setPlusPacket(await api.manualPlusPacket(task.id)) }
    catch (error) { setPlusNotice(error instanceof Error ? error.message : '分析包生成失败，请稍后重试。') }
    finally { setBusy(false) }
  }
  const copyPlusPrompt = async () => {
    if (!plusPacket) return
    try { await navigator.clipboard.writeText(plusPacket.prompt); setPlusNotice('已复制。现在粘贴到 ChatGPT Plus，完成后把它返回的原始 JSON 或 CSV 粘到下方。') }
    catch { setPlusNotice('浏览器未允许自动复制，请在文本框中全选后手动复制。') }
  }
  const importPlusResult = async () => {
    if (!plusTask || !plusOutput.trim()) return
    setBusy(true); setPlusNotice('正在导入候选并准备官网核验……')
    try {
      const imported = await api.importManualPlusResult(plusTask.id, plusOutput)
      const started = await api.runApplicationDiscoveryTask(plusTask.id)
      const updated = await load(documentId)
      const refreshedTask = updated.find(item => item.id === plusTask.id)
      if (refreshedTask) focusTaskLeads(refreshedTask)
      setPlusTask(null); setPlusPacket(null); setPlusOutput('')
      setNotice(`${imported.message} ${started.message} 下方线索库已自动切换到这个产品。`)
    } catch (error) {
      setPlusNotice(error instanceof Error ? error.message : '导入失败，请检查返回格式。')
    } finally { setBusy(false) }
  }
  ...
  return <section className="tds-discovery panel">
    ...
    {tasks.length > 0 && <section className="tds-task-list"><header><div><h3>4. 真实采集、Plus 人工辅助与客户清单</h3><p>纯爬虫入口不足时，可复制完整分析包到你的 ChatGPT Plus；粘回候选后，网站会自动去重并继续核验官网、业务证据和公开联系方式。</p></div><button className="secondary" disabled={busy} onClick={() => void work(async () => { await load(documentId); return '任务状态已刷新。' })}><RefreshCw size={14}/> 刷新状态</button></header>{tasks.map(task => <article key={task.id}><div><b>{task.task_name}</b><p>{task.target_region || '全球'} · {task.search_provider || '未配置采集入口'} · {task.status}</p><small>{task.provider_notice || '应用快照已锁定。'}{task.failure_message ? ` · ${task.failure_message}` : ''}</small></div><dl><div><dt>发现</dt><dd>{task.discovered_count}</dd></div><div><dt>官网核验</dt><dd>{task.verified_count}</dd></div><div><dt>应用相关</dt><dd>{task.matched_count}</dd></div><div><dt>有联系方式</dt><dd>{task.contact_count}</dd></div></dl><div className="tds-task-actions"><button className="primary" disabled={busy || task.status === '待配置' || task.status === '运行中'} onClick={() => runTask(task)}><Play size={14}/> {task.status === '已完成' || task.status === '部分失败' ? '再次运行' : '开始真实发现'}</button><button className="secondary plus-copy-button" disabled={busy || task.status === '运行中'} onClick={() => void openManualPlus(task)}><ClipboardCopy size={14}/> 复制给 ChatGPT 分析</button>{task.legacy_lead_task_id && <button className="secondary" disabled={busy} onClick={() => focusTaskLeads(task)}><Search size={14}/> 查看本产品需求客户</button>}<button className="secondary" disabled={busy} onClick={() => void openMap(task)}><MapPinned size={14}/> 客户地图 {task.discovered_count || ''}</button><button className="secondary" disabled={busy || task.discovered_count === 0} onClick={() => void work(async () => { await api.exportCustomerLeadsCsv(`application_task_id=${encodeURIComponent(task.id)}`); return '客户清单已导出。' })}><Download size={14}/> 导出清单</button><button className="danger" disabled={busy} onClick={() => deleteTask(task)}><Trash2 size={14}/> 删除任务</button></div></article>)}</section>}
    {plusTask && <div className="modal-layer manual-plus-layer"><section className="modal manual-plus-modal" role="dialog" aria-modal="true" aria-label="ChatGPT Plus 人工辅助"><div className="modal-head"><div><p className="eyebrow"><ClipboardCopy size={13}/> NO API FEE</p><h2>ChatGPT Plus 人工辅助</h2><small>{plusTask.task_name}</small></div><button className="close" disabled={busy} aria-label="关闭" onClick={() => { setPlusTask(null); setPlusPacket(null); setPlusOutput('') }}><X size={18}/></button></div><ol className="manual-plus-steps"><li><b>复制分析包</b><span>已包含 TDS 确认应用、目标企业类型、地区、搜索词和已有候选。</span></li><li><b>在 ChatGPT Plus 中运行</b><span>让它实际研究需求端公司，并只返回规定的 JSON。</span></li><li><b>粘回并核验</b><span>网站自动去重，然后启动爬虫核验官网、业务和公开联系方式。</span></li></ol>{plusNotice && <p className="compliance-note">{plusNotice}</p>}<label className="manual-plus-field"><span>给 ChatGPT Plus 的完整分析指令</span><textarea readOnly value={plusPacket?.prompt || '正在生成分析包……'} onFocus={event => event.currentTarget.select()}/></label><button className="secondary manual-plus-copy" disabled={busy || !plusPacket} onClick={() => void copyPlusPrompt()}><ClipboardCopy size={14}/> 一键复制分析包</button><label className="manual-plus-field"><span>粘贴 ChatGPT 返回的原始 JSON 或 CSV</span><textarea value={plusOutput} disabled={busy} onChange={event => setPlusOutput(event.target.value)} placeholder={'推荐直接粘贴 JSON；也支持带标题行的 CSV。\n不要粘贴 Markdown 代码围栏。'}/></label><div className="manual-plus-safety">导入结果只进入“待补信息”，评分最高暂记 40；爬虫核验前不会进入严格客户名单或 CRM，也不会发送邮件。</div><div className="form-actions"><button disabled={busy} onClick={() => { setPlusTask(null); setPlusPacket(null); setPlusOutput('') }}>取消</button><button className="primary" disabled={busy || !plusPacket || !plusOutput.trim()} onClick={() => void importPlusResult()}><Play size={14}/> 导入并启动官网核验</button></div></section></div>}
