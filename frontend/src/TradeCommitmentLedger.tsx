...
const productName = (id: string | null | undefined, products: Product[]) => {
  ...
  return product.product_code === product.product_name ? product.product_code : `${product.product_code} · ${product.product_name}`
}
...
export function TradeCommitmentLedger({ commitments, customers, projects, products, create, update }: {
  ...
  return <section className="page commitment-page">
    ...
    <div className="commitment-list">{visible.map(item => { const customer = customers.find(row => row.id === item.customer_id); const state = dueState(item); return <article className={`commitment-card ${state}`} key={item.id}>
      <header><div className="commitment-card-main"><small>{customer ? `${countryName(customer.country)} · ${customer.contact_person || '联系人待补充'}` : '历史客户'}</small><h2>{item.title}</h2><div className="commitment-context"><span title={customerName(item.customer_id, customers)}><i>客户</i>{customerName(item.customer_id, customers)}</span>{item.project_id && <span title={projectName(item.project_id, projects)}><i>项目</i>{projectName(item.project_id, projects)}</span>}{item.product_id && <span title={productName(item.product_id, products)}><i>产品</i>{productName(item.product_id, products)}</span>}</div></div><div className="commitment-card-tags"><span className={`commitment-status ${item.status}`}>{item.status}</span><span>{item.responsible_party}</span></div></header>
