...
export type LeadSearchTask = {
  id: string; user_id?: string; task_name: string; discovery_mode?: '需求客户' | '供应工厂'; discovery_strategy?: 'public_seed_crawl' | 'search_plus_crawl'; product_id?: string | null; product_keywords: string[]; application_keywords: string[]; target_countries: string[]; excluded_countries: string[]; target_company_types: string[]; profile_exclusion_rules: string[]; source_urls: string[]; search_language: string; max_results: number; daily_enabled: boolean; daily_run_time: string; status: '启用' | '暂停'; run_state?: '草稿' | '排队中' | '搜索中' | '抓取中' | '分析中' | '已暂停' | '已完成' | '部分失败' | '已取消'; pause_requested?: boolean; cancel_requested?: boolean; current_url?: string | null; last_progress?: { discovered?: number; inserted?: number; skipped?: number; current_url?: string }; minimum_match_score?: number; max_pages_per_domain?: number; extract_public_email?: boolean; visit_contact_page?: boolean; ai_enabled?: boolean; task_note?: string | null; last_run_at?: string | null; last_run_status?: '成功' | '失败' | '跳过' | null; last_error?: string | null; created_at: string
}
export type CustomerLead = {
  ...
  development_campaign_id?: string | null
  development_status?: '发现' | '待核实' | '合格' | '不匹配' | '已联系' | '已回复' | '拒绝联系'
}
...
export type ProductKeyword = { id: string; product_id: string; keyword: string; keyword_type: 'include' | 'exclude' | 'local'; language_code: string; country?: string | null; weight: number; enabled: boolean; created_at?: string; updated_at?: string }
export type CrawlSource = { id: string; name: string; source_type: '官网' | '展会目录' | '协会目录' | '行业目录' | '政府或商会目录' | '用户导入' | '合规搜索 API'; start_url?: string | null; country?: string | null; industry?: string | null; enabled: boolean; request_delay_seconds: number; max_pages: number; last_run_at?: string | null; success_count: number; failure_count: number; created_at?: string; updated_at?: string }
export type DomainBlock = { id: string; root_domain: string; reason: string; enabled: boolean; created_at?: string }
export type DevelopmentCampaign = { id: string; campaign_name: string; product_code: string; product_name: string; product_claim_text: string; product_claim_source: string; target_country: string; target_company_types: string[]; applications: string[]; exclusion_terms: string[]; daily_candidate_limit: number; status: '草稿' | '启用' | '暂停' | '已关闭'; created_at: string; updated_at: string }
export type DiscoveryQuery = { id: string; campaign_id: string; query_text: string; country: string; query_kind: 'web' | 'maps' | 'linkedin' | 'pdf_directory'; search_url: string; executed_at?: string | null; result_url?: string | null; note?: string | null; created_at: string }
export type SourceDocument = { id: string; campaign_id: string; source_name: string; source_url?: string | null; source_type: 'CSV' | 'PDF' | '手工网页结果'; page_count?: number | null; imported_at: string }
export type LeadEvidence = { id: string; customer_lead_id: string; source_document_id?: string | null; evidence_type: '官网' | 'PDF' | '搜索结果' | '人工核查'; source_url?: string | null; page_number?: number | null; excerpt: string; evidence_role: '应用或产品' | '客户身份' | '国家或地址' | '公开联系入口' | '近期活动' | '发现来源'; verified_at?: string | null; created_at: string }
export type LeadContact = { id: string; customer_lead_id: string; contact_name?: string | null; job_title?: string | null; department?: string | null; email?: string | null; email_status: '官网公开' | '已验证' | '未验证' | '不可发送'; source_url?: string | null; verified_at?: string | null; created_at: string }
export type OutreachDraft = { id: string; campaign_id: string; customer_lead_id: string; lead_contact_id?: string | null; subject: string; channel: '邮件' | 'LinkedIn' | 'WhatsApp' | 'Facebook'; recipient?: string | null; company_fact: string; fact_source_url?: string | null; product_code: string; product_claim_source: string; draft_body: string; approval_state: '待审核' | '已审核' | '已拒绝'; approval_note?: string | null; approved_at?: string | null; sent_at?: string | null; reply_state: '未发送' | '待回复' | '已回复' | '拒绝联系' | '退信'; next_follow_up_at?: string | null; created_at: string; updated_at: string }
export type DevelopmentWorkspace = { campaign: DevelopmentCampaign; queries: DiscoveryQuery[]; documents: SourceDocument[]; leads: CustomerLead[]; evidences: LeadEvidence[]; contacts: LeadContact[]; drafts: OutreachDraft[] }
export type TdsDocument = { id: string; product_id?: string | null; original_file_name: string; document_version?: string | null; mime_type: string; byte_size: number; content_sha256: string; parse_status: '已解析' | '需要 OCR' | '解析失败'; parse_error?: string | null; extracted_summary?: string | null; parsed_at: string; created_at: string }
export type TdsPreset = { id: 'nl-w1201' | 'epoxidized-linseed-oil' | 'fertilizer-coating'; title: string; material: string; summary: string; original_file_name: string; document_version: string; application_count: number; applications: Array<{ application_name: string; description: string; target_company_types: string[] }> }
export type TdsApplication = { id: string; tds_document_id: string; application_name: string; description?: string | null; substrate_or_object?: string | null; material_function?: string | null; process_conditions?: string | null; limitations?: string | null; evidence_excerpt?: string | null; evidence_page?: number | null; evidence_status: 'TDS明确' | '推测待确认' | '用户补充'; target_company_types: string[]; official_business_evidence?: string | null; exclusion_notes?: string | null; search_terms: string[]; local_search_terms: string[]; selected: boolean; enabled: boolean; revision_note?: string | null; updated_at: string; created_at: string }
export type ApplicationDiscoveryTask = { id: string; tds_document_id: string; task_name: string; target_region?: string | null; candidate_limit: number; search_budget: number; application_snapshot: TdsApplication[]; status: '草稿' | '待配置' | '待运行' | '运行中' | '已完成' | '部分失败' | '已取消'; search_provider?: string | null; provider_notice?: string | null; legacy_lead_task_id?: string | null; discovered_count: number; verified_count: number; matched_count: number; contact_count: number; failure_message?: string | null; created_at: string; updated_at: string }
export type ApplicationDiscoveryQuery = { id: string; application_task_id: string; tds_application_id?: string | null; query_text: string; query_language: string; query_kind: 'web' | 'pdf_directory' | 'maps' | 'local'; execution_status: '待配置' | '待运行' | '已运行' | '失败'; result_count: number; error_message?: string | null; created_at: string }
export type ApplicationDiscoveryWorkspace = { task: ApplicationDiscoveryTask; queries: ApplicationDiscoveryQuery[]; matches: { id: string; customer_lead_id: string; application_task_id: string; tds_application_id?: string | null; match_status: string; matching_reason?: string | null; pending_confirmation?: string | null; evidence_strength: number }[]; leads: CustomerLead[] }
export type ManualPlusPacket = { task_id: string; format_version: string; candidate_limit: number; current_candidate_count: number; prompt: string; message: string }
export type ManualPlusImportResult = { inserted: number; updated: number; preserved: number; legacy_lead_task_id: string; message: string }
