...
export type ApplicationDiscoveryTask = { id: string; tds_document_id: string; task_name: string; target_region?: string | null; candidate_limit: number; search_budget: number; application_snapshot: TdsApplication[]; status: '草稿' | '待配置' | '待运行' | '运行中' | '已完成' | '部分失败' | '已取消'; search_provider?: string | null; provider_notice?: string | null; discovered_count: number; verified_count: number; matched_count: number; contact_count: number; failure_message?: string | null; created_at: string; updated_at: string }
export type ApplicationDiscoveryQuery = { id: string; application_discovery_task_id: string; tds_application_id?: string | null; query_text: string; query_language: string; query_kind: 'web' | 'pdf_directory' | 'maps' | 'local'; execution_status: '待配置' | '待运行' | '已运行' | '失败'; result_count: number; error_message?: string | null; created_at: string }
