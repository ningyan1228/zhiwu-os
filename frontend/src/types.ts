...
export type LeadSearchTask = {
  id: string; user_id?: string; task_name: string; product_keywords: string[]; application_keywords: string[]; target_countries: string[]; excluded_countries: string[]; target_company_types: string[]; source_urls: string[]; search_language: string; max_results: number; daily_enabled: boolean; daily_run_time: string; status: '启用' | '暂停'; last_run_at?: string | null; last_run_status?: '成功' | '失败' | '跳过' | null; last_error?: string | null; created_at: string
}
