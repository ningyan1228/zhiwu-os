import type { ApplicationDiscoveryQuery, ApplicationDiscoveryTask, ApplicationDiscoveryWorkspace, CrawlSource, Customer, CustomerLead, DailyLog, DevelopmentCampaign, DevelopmentWorkspace, DiscoveryQuery, DomainBlock, EmailSync, Followup, ImportApplyResult, ImportBatch, ImportPreviewResult, LeadDiscoveryRun, LeadSearchTask, MailAiFactCard, MailboxAccount, MailEmail, MailReplyDraft, ManualPlusImportResult, ManualPlusPacket, OutreachDraft, Product, ProductCustomerRelation, ProductKeyword, Project, Quote, SalesOrder, StrictLeadImportResult, Supplier, SupplierContact, SupplierDocument, SupplierFollowup, SupplierInsight, SupplierProduct, SupplierProjectLink, SupplierRfq, Task, TdsApplication, TdsDocument, TdsPreset, TimelineEvent, TradeCommitment, WorkspaceMember } from './types'
...
export const api = {
  ...
  emailAiFactCard: (id: string) => request<MailAiFactCard | null>(`/api/emails/${id}/ai-fact-card`),
  emailAiFactCards: (ids: string[]) => ids.length ? request<MailAiFactCard[]>(`/api/mail-ai-fact-cards?email_ids=${encodeURIComponent(ids.join(','))}`) : Promise.resolve([]),
  generateEmailAiFactCard: (id: string) => request<MailAiFactCard>(`/api/emails/${id}/ai-fact-card`, { method: 'POST' }),
  generateEmailReplyDraft: (id: string, purpose?: string) => request<MailReplyDraft>(`/api/emails/${id}/reply-draft`, { method: 'POST', body: JSON.stringify({ purpose: purpose || null }) }),
  reviewEmailAiFactCard: (id: string, status: '待审核' | '已忽略', review_note?: string) => request<MailAiFactCard>(`/api/emails/${id}/ai-fact-card`, { method: 'PATCH', body: JSON.stringify({ status, review_note }) }),
  ...
  tdsDocuments: () => request<TdsDocument[]>('/api/tds-documents'),
  tdsPresets: () => request<TdsPreset[]>('/api/tds-presets'),
  bootstrapTdsPreset: (presetId: TdsPreset['id']) => request<{ document: TdsDocument; applications: TdsApplication[]; reused: boolean; message: string }>(`/api/tds-presets/${presetId}/bootstrap`, { method: 'POST' }),
  parseTdsDocument: (file: File, payload: { product_id?: string; document_version?: string } = {}) => { const form = new FormData(); form.append('file', file); if (payload.product_id) form.append('product_id', payload.product_id); if (payload.document_version) form.append('document_version', payload.document_version); return upload<{ document: TdsDocument; applications: TdsApplication[]; reused: boolean; message: string }>('/api/tds-documents/parse', form) },
  tdsApplications: (documentId: string) => request<TdsApplication[]>(`/api/tds-documents/${documentId}/applications`),
  createTdsApplication: (documentId: string, payload: Omit<TdsApplication, 'id' | 'tds_document_id' | 'updated_at' | 'created_at'>) => request<TdsApplication>(`/api/tds-documents/${documentId}/applications`, { method: 'POST', body: JSON.stringify(payload) }),
  ...
  deleteApplicationDiscoveryTask: (taskId: string) => request<{ deleted: boolean; task_id: string; message: string }>(`/api/application-discovery-tasks/${taskId}`, { method: 'DELETE' }),
  ...
  manualPlusPacket: (taskId: string) => request<ManualPlusPacket>(`/api/application-discovery-tasks/${taskId}/chatgpt-packet`),
  importManualPlusResult: (taskId: string, rawText: string) => request<ManualPlusImportResult>(`/api/application-discovery-tasks/${taskId}/chatgpt-import`, { method: 'POST', body: JSON.stringify({ raw_text: rawText }) }),
